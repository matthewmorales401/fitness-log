...
document.getElementById('modal-date').valueAsDate = newUTCDate()
...
.then(response => response.json())
.then(data => {
    console.log("Reminder success. This is the most recently planned activity:", data.recentActivity);

    if (data.recentActivity != null) {
      if (data.recentActivity.activity == 'Yoga'){
        data.recentActivity.activity = 'do yoga';
      }
      else if (data.recentActivity.activity == 'Basketball' || data.recentActivity.activity == 'Soccer'){
        data.recentActivity.activity = 'play ' + data.recentActivity.activity;
      }
      document.getElementById('reminder-container').style.display = "block";
      document.getElementById('rem-act').textContent = data.recentActivity.activity.toLowerCase();

      
      let dayOfWeek = ["on Monday", "on Tuesday", "on Wednesday", "on Thursday", "on Friday", "on Saturday", "on Sunday"];

      let date = new Date(data.recentActivity.date);
    ...
    document.getElementById("yes").addEventListener("click", function() {
      ...
      document.getElementById("pAct-activity").value = data.recentActivity.activity;
      let dateObj = new Date(data.recentActivity.date);
    let month = dateObj.getUTCMonth() + 1; //months starts from 1
    let day = dateObj.getUTCDate();
    let year = dateObj.getUTCFullYear();

    let pastDate = year + "-" + month + "-" + day;
      document.getElementById("pAct-date").textContent = pastDate;
